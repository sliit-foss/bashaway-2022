    ...
    return <div className="flex flex-col bg-black h-full justify-center items-center pt-24 px-6">
        ...
        <Title title="Timeline"/>
        {
            timelineList.map((list) => {
                return (
                    <>
                        <span className={`${list.isSmallText ? "text-[20px]" : "text-[32px]"} mt-[30px] text-white text-center font-semibold leading-6"`}>{list.title}</span>
                        <span className="text-gray-500 font-normal text-center text-sm mt-2">{list.dateTime}</span>
                        <div className="mt-5">
